    ...
    <div class="relative z-10 w-full max-w-lg flex flex-col items-center gap-10">
      ...
      <div class="w-full bg-[var(--surface-1)] border border-[var(--border-default)] shadow-2xl p-8 rounded-[var(--radius-lg)]">
        <div class="flex flex-col md:flex-row items-center gap-8">
          ...
          <div class="flex-1 min-w-0 flex flex-col items-center md:items-start text-center md:text-left">
            <span class="mb-1 text-[10px] font-bold uppercase tracking-widest text-[var(--accent)]">{{ t('liteMode.nowPlaying') }}</span>
            <h2 class="text-2xl font-bold text-[var(--text-primary)] leading-tight mb-1 line-clamp-2">
              {{ nowPlaying?.music.name || t('liteMode.roomIdle') }}
            </h2>
      ...
      <label class="flex items-center gap-3 cursor-pointer group select-none">
        ...
        <span class="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-wider">
          {{ t('liteMode.autoLiteWhenInactive') }}
        </span>
      </label>
      ...
      >
        ...
        <span class="text-xs tracking-[0.2em] uppercase">{{ t('liteMode.returnToMainView') }}</span>
      </button>
    </div>
  </div>
</template>
...
const artistLine = computed(() => Array.isArray(props.nowPlaying?.music?.artists) && props.nowPlaying.music.artists.length
  ? props.nowPlaying.music.artists.join(' / ')
  : t('app.brand'));

  ...
  <div class="w-full h-full flex items-center justify-between px-6">
    ...
    <div class="flex flex-col items-center justify-center w-1/3 gap-2">
      ...
      <div class="flex items-center gap-4 text-sm font-bold">
        <button @click="player.toggleShuffle" class="text-[#8A8A8A] hover:text-white transition-colors" :class="{ 'text-[#D3C2F3]': player.isShuffle }">Shuf</button>
        <button class="text-[#8A8A8A] hover:text-white transition-colors opacity-50 cursor-not-allowed">Prev</button>
        <button @click="player.togglePause" class="text-black bg-[#D3C2F3] px-3 py-1 rounded hover:bg-white transition-colors">
          {{ player.isPaused ? 'Play' : 'Pause' }}
        </button>
        <button @click="player.playNext" class="text-[#8A8A8A] hover:text-white transition-colors">Next</button>
      </div>
      <!-- Progress -->
    ...
    <div class="w-1/3 flex items-center justify-end gap-4">
       <!-- Volume / Extra -->
       <button @click="player.sendLike" class="text-[#8A8A8A] hover:text-[#D3C2F3] text-sm font-bold transition-colors">Like</button>
    </div>
  </div>
</template>

...
本仓库是基于上游 MusicParty 的个人 fork。NRT-Base 分支重点补强了前端 UI/UX、移动端体验、国际化、播放同步、队列操作、专辑导入、直播流稳定性、可访问性、前端构建体积和可选 Navidrome 私有曲库接入。
...
- **桌面与移动端 UI**：桌面三栏主界面支持全局缩放与主舞台比例调节，避免桌面横向溢出；移动端提供底部导航与独立播放/队列/搜索/聊天视图。
- **可访问控件**：桌面壳、播放控制、队列/歌单图标按钮和可点击曲目行提供可读的 accessible name、键盘焦点状态与按钮语义。
- **歌词体验**：支持歌词和翻译歌词展示，移动端有独立的歌词面板和字号/对齐控制。
- **实时互动**：聊天室、系统消息、在线成员、点赞反馈和直播流听众计数。
- **账号与房间控制**：首次启动创建管理员账号；后续使用账号密码登录，管理员可管理房间、音源、本地曲库、权限锁定和 Cookie。个人资料页支持显示名修改、密码修改和清晰的退出登录确认按钮。
- **可选 HTTP 直播流**：通过 FFmpeg 输出 `/radio/stream`，适合在 VRChat 等外部场景收听。
- **i18n、本地字体与构建分包**：前端接入 `vue-i18n`，Material Symbols 字体已本地化；Vite 生产构建会拆分 Vue、网络、UI、拖拽和工具依赖，降低入口 chunk 体积。

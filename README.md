...
在项目根目录双击 `start-dev.cmd`，或者在 PowerShell 中执行：

```powershell
.\start-dev.ps1
```

如果本机安装了 Git for Windows，`start-dev.cmd` 和 `start-netease-api.cmd` 现在会优先通过 Git Bash 启动，只有在找不到 Git Bash 时才回退到 PowerShell。
...
启动后会打开两个命令窗口，分别运行后端和前端。报错会直接显示在对应窗口中，前端就绪后会自动打开浏览器。

如果你想在 Windows 本地一并使用非 Docker 的 `NeteaseCloudMusicApi`，可以先单独执行：

```powershell
.\start-netease-api.ps1
```

或直接联动启动：

```powershell
.\start-dev.ps1 -StartNeteaseApi
```

说明：

- `start-netease-api.ps1` / `start-netease-api.cmd` 会通过 `npx -y NeteaseCloudMusicApi@latest` 在本机启动 API。
- 新增的 `start-dev.sh` / `start-netease-api.sh` 是 Git Bash 优先方案，Windows 下推荐优先使用这套。
- 默认端口是 `3000`，可通过 `-Port 3001` 覆盖。
- `start-dev.ps1` 现在会自动把后端的 `NETEASE_API_URL` 指向本地 `http://127.0.0.1:3000`，也可通过 `-NeteaseApiUrl` 自定义。
- Git Bash 方案会把后端输出写入固定日志文件：`.dev-logs/backend-dev.log`，每次启动覆盖为当前会话内容。
- Git Bash 方案现在也会把本地 `NeteaseCloudMusicApi` 输出写入：`.dev-logs/netease-api.log`。
- 本地非 Docker 调试时，网易云 Cookie 仍然是写给 **Music Party 后端** 的，不是写给前端。
- 现在脚本会优先读取项目根目录下的 `cookies.json`。可以先复制 `cookies.example.json` 为 `cookies.json`，然后填写：
...
```powershell
$env:NETEASE_COOKIE="MUSIC_U=xxxx...; __csrf=xxxx..."
.\start-dev.ps1 -StartNeteaseApi
```
...
```powershell
.\start-dev.ps1 -StartNeteaseApi -NeteaseCookie "MUSIC_U=xxxx...; __csrf=xxxx..."
```

- 如果应用已经启动，也可以继续用搜索框管理员命令动态更新：
  `//COOKIE netease <cookie_string>`
